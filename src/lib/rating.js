...
export async function getRatingLine({imdbId, type, season, episode}){
  ...
  // per-episode IMDb. All three ratings come from the single (shared, cached) OMDb record.
  if(config.omdbApiKey){
    const record = await omdb.getById(imdbId, isEpisode ? {season, episode} : {}).catch(() => null);
    if(record){
      imdb = record.imdbRating;
      rt = record.rt;
      mc = record.mc;
    }
  }
